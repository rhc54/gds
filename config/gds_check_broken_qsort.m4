...
dnl the gds_qsort instead.

# check for broken qsort
# GDS_CHECK_BROKEN_QSORT(prefix, [action-if-found], [action-if-not-found])
# --------------------------------------------------------
AC_DEFUN([GDS_CHECK_BROKEN_QSORT],[
  ...
  AC_DEFINE_UNQUOTED([GDS_HAVE_BROKEN_QSORT], [$define_result],
