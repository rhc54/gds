...
# GDS_C_GET_ALIGN(type, config_var)
# ----------------------------------
...
AC_DEFUN([GDS_C_GET_ALIGNMENT],[
    AC_CACHE_CHECK([alignment of $1],
                   [AS_TR_SH([gds_cv_c_align_$1])],
...
[[
    ...
    fprintf(f, "%d\n", (diff >= 0) ? diff : -diff);
]])],                         [AS_TR_SH([gds_cv_c_align_$1])=`cat conftestval`],
                               ...
                               [ # cross compile - do a non-executable test.  Trick
                                 ...
                                 _AC_COMPUTE_INT([(long int) offsetof (gds__type_alignof_, y)],
                                                 [AS_TR_SH([gds_cv_c_align_$1])],
                                                 [AC_INCLUDES_DEFAULT
#include <stdbool.h>
...
#endif
typedef struct { char x; $1 y; } gds__type_alignof_;
],
                                                 [AC_MSG_WARN([*** Problem running configure test!])
                                                  AC_MSG_WARN([*** See config.log for details.])
                                                  AC_MSG_ERROR([*** Cannot continue.])])])])

AC_DEFINE_UNQUOTED([$2], [$AS_TR_SH([gds_cv_c_align_$1])], [Alignment of type $1])
eval "$2=$AS_TR_SH([gds_cv_c_align_$1])"
