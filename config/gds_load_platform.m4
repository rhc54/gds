...
AC_DEFUN([GDS_LOAD_PLATFORM], [
    ...
    if test "$with_platform" = "yes" ; then
        ...
        GDS_LOG_MSG([Loading environment file $with_platform, with contents below])
        GDS_LOG_FILE([$with_platform])
        ...
        if test "$GDS_PLATFORM_LOADED" != "" ; then
           platform_loaded="$GDS_PLATFORM_LOADED"
        else
           platform_loaded="$with_platform"
        fi
        echo "Loaded platform arguments for $platform_loaded"
        GDS_LOG_MSG([Loaded platform arguments for $platform_loaded])
