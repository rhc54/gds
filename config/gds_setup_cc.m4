...
AC_DEFUN([GDS_SETUP_CC],[
    ...
    AC_REQUIRE([_GDS_START_SETUP_CC])
    AC_REQUIRE([_GDS_PROG_CC])
    ...
    if test "x$ac_cv_prog_cc_c99" = xno ; then
        AC_MSG_WARN([GDS requires a C99 compiler])
        AC_MSG_ERROR([Aborting.])
    fi


    GDS_C_COMPILER_VENDOR([gds_c_vendor])
    ...
    AS_IF([test "$gds_cv_c_compiler_vendor" != "ibm"],
...
# undef _GNU_SOURCE
    ...
    GDS_CFLAGS_BEFORE_PICKY="$CFLAGS"
    ...
    if test $WANT_PICKY_COMPILER -eq 1; then
        ...
        if test "$gds_cv_cc_wno_long_double" = "yes" ; then
    ...
    if test "$GCC" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CC supports -finline-functions],
                   [gds_cv_cc_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [gds_cv_cc_finline_functions="yes"
                                    if test -s conftest.err ; then
                                        for i in unused 'not supported' ; do
                                            if $GREP -iq "$i" conftest.err; then
                                                gds_cv_cc_finline_functions="no"
                                                break;
                                            fi
                                        done
                                    fi
                                   ...
                                   [gds_cv_cc_finline_functions="no"])])
        if test "$gds_cv_cc_finline_functions" = "yes" ; then
        ...
        AC_CACHE_CHECK([if $CC supports -fno-strict-aliasing],
                   [gds_cv_cc_fno_strict_aliasing],
                   [AC_TRY_COMPILE([], [],
                                   [gds_cv_cc_fno_strict_aliasing="yes"],
                                   [gds_cv_cc_fno_strict_aliasing="no"])])
        if test "$gds_cv_cc_fno_strict_aliasing" = "yes" ; then
    ...
    if test ! -z "$RESTRICT_CFLAGS" ; then
        ...
        AC_CACHE_CHECK([if $CC supports $RESTRICT_CFLAGS],
                   [gds_cv_cc_restrict_cflags],
                   [AC_TRY_COMPILE([], [],
                                   [gds_cv_cc_restrict_cflags="yes"],
                                   [gds_cv_cc_restrict_cflags="no"])])
        if test "$gds_cv_cc_restrict_cflags" = "yes" ; then
            add=" $RESTRICT_CFLAGS"
        fi
    ...
    AC_CACHE_CHECK([if $CC supports __builtin_expect],
        [gds_cv_cc_supports___builtin_expect],
        [AC_TRY_LINK([],
          [void *ptr = (void*) 0;
           if (__builtin_expect (ptr != (void*) 0, 1)) return 0;],
          [gds_cv_cc_supports___builtin_expect="yes"],
          [gds_cv_cc_supports___builtin_expect="no"])])
    if test "$gds_cv_cc_supports___builtin_expect" = "yes" ; then
        have_cc_builtin_expect=1
    else
        have_cc_builtin_expect=0
    fi
    AC_DEFINE_UNQUOTED([GDS_C_HAVE_BUILTIN_EXPECT], [$have_cc_builtin_expect],
        [Whether C compiler supports __builtin_expect])

    # see if the C compiler supports __builtin_prefetch
    AC_CACHE_CHECK([if $CC supports __builtin_prefetch],
        [gds_cv_cc_supports___builtin_prefetch],
        [AC_TRY_LINK([],
          [int ptr;
           __builtin_prefetch(&ptr,0,0);],
          [gds_cv_cc_supports___builtin_prefetch="yes"],
          [gds_cv_cc_supports___builtin_prefetch="no"])])
    if test "$gds_cv_cc_supports___builtin_prefetch" = "yes" ; then
        have_cc_builtin_prefetch=1
    else
        have_cc_builtin_prefetch=0
    fi
    AC_DEFINE_UNQUOTED([GDS_C_HAVE_BUILTIN_PREFETCH], [$have_cc_builtin_prefetch],
        [Whether C compiler supports __builtin_prefetch])

    # see if the C compiler supports __builtin_clz
    AC_CACHE_CHECK([if $CC supports __builtin_clz],
        [gds_cv_cc_supports___builtin_clz],
        [AC_TRY_LINK([],
            [int value = 0xffff; /* we know we have 16 bits set */
             if ((8*sizeof(int)-16) != __builtin_clz(value)) return 0;],
            [gds_cv_cc_supports___builtin_clz="yes"],
            [gds_cv_cc_supports___builtin_clz="no"])])
    if test "$gds_cv_cc_supports___builtin_clz" = "yes" ; then
        have_cc_builtin_clz=1
    else
        have_cc_builtin_clz=0
    fi
    AC_DEFINE_UNQUOTED([GDS_C_HAVE_BUILTIN_CLZ], [$have_cc_builtin_clz],
    ...
    GDS_ENSURE_CONTAINS_OPTFLAGS("$GDS_CFLAGS_BEFORE_PICKY")
    GDS_CFLAGS_BEFORE_PICKY="$co_result"

    AC_MSG_CHECKING([for C optimization flags])
    GDS_ENSURE_CONTAINS_OPTFLAGS(["$CFLAGS"])
    ...
    GDS_CHECK_IDENT([CC], [CFLAGS], [c], [C])

])


AC_DEFUN([_GDS_START_SETUP_CC],[
    gds_show_subtitle "C compiler and preprocessor"
...
AC_DEFUN([_GDS_PROG_CC],[
    #
    # Check for the compiler
    #
    GDS_VAR_SCOPE_PUSH([gds_cflags_save dummy gds_cc_arvgv0])
    gds_cflags_save="$CFLAGS"
    AC_PROG_CC
    BASECC="`basename $CC`"
    CFLAGS="$gds_cflags_save"
    AC_DEFINE_UNQUOTED(GDS_CC, "$CC", [GDS underlying C compiler])
    set dummy $CC
    gds_cc_argv0=[$]2
    GDS_WHICH([$gds_cc_argv0], [GDS_CC_ABSOLUTE])
    AC_SUBST(GDS_CC_ABSOLUTE)
    GDS_VAR_SCOPE_POP
])
