...
GDS_CONFIGURE_USER="`whoami`"
GDS_CONFIGURE_HOST="`hostname | head -n 1`"
GDS_CONFIGURE_DATE="`date`"

#
# Save these details so that they can be used in gds_info later
#
AC_SUBST(GDS_CONFIGURE_USER)
AC_SUBST(GDS_CONFIGURE_HOST)
AC_SUBST(GDS_CONFIGURE_DATE)])dnl

dnl #######################################################################
dnl #######################################################################
dnl #######################################################################

AC_DEFUN([GDS_BASIC_SETUP],[
#
# Save some stats about this build
#
...
#
# See if we can find an old installation of GDS to overwrite
#

# Stupid autoconf 2.54 has a bug in AC_PREFIX_PROGRAM -- if gds_clean
# is not found in the path and the user did not specify --prefix,
# we'll get a $prefix of "."

gds_prefix_save="$prefix"
AC_PREFIX_PROGRAM(gds_clean)
if test "$prefix" = "."; then
    prefix="$gds_prefix_save"
fi
unset gds_prefix_save
...
if test "$program_prefix$program_suffix$program_transform_name" != "NONENONEs,x,x," ; then
    AC_MSG_WARN([*** The GDS configure script does not support --program-prefix, --program-suffix or --program-transform-name. Users are recommended to instead use --prefix with a unique directory and make symbolic links as desired for renaming.])
...
for val in ${$1}; do
    ...
    gds_done="`expr $gds_i \> $gds_count`"
    while test "$gds_found" = "0" && test "$gds_done" = "0"; do

	# Have we seen this token already?  Prefix the comparison with
	# "x" so that "-Lfoo" values won't be cause an error.

	gds_eval="expr x$val = x\$gds_array_$gds_i"
	gds_found=`eval $gds_eval`
    ...
    -Xclang)
            gds_found=0
            gds_i=`expr $gds_count + 1`
            ;;
    esac

    # If we didn't find the token, add it to the "array"

    if test "$gds_found" = "0"; then
	gds_eval="gds_array_$gds_i=$val"
	eval $gds_eval
	gds_count="`expr $gds_count + 1`"
    else
	gds_i="`expr $gds_i - 1`"
    fi
done
...
gds_done="`expr $gds_i \> $gds_count`"
gds_newval=
while test "$gds_done" = "0"; do
    gds_eval="gds_newval=\"$gds_newval \$gds_array_$gds_i\""
    eval $gds_eval

    gds_eval="unset gds_array_$gds_i"
    eval $gds_eval
...
gds_newval="`echo $gds_newval`"
gds_eval="$gds_name=\"$gds_newval\""
eval $gds_eval

# Clean up

unset gds_name gds_i gds_done gds_newval gds_eval gds_count])dnl

dnl #######################################################################
dnl #######################################################################
dnl #######################################################################

# GDS_APPEND_UNIQ(variable, new_argument)
# ----------------------------------------
# Append new_argument to variable if not already in variable.  This assumes a
# space seperated list.
#
# This could probably be made more efficient :(.
AC_DEFUN([GDS_APPEND_UNIQ], [
for arg in $2; do
    gds_found=0;
    for val in ${$1}; do
...
# GDS_FLAGS_APPEND_UNIQ(variable, new_argument)
# ----------------------------------------------
...
AC_DEFUN([GDS_FLAGS_APPEND_UNIQ], [
    GDS_VAR_SCOPE_PUSH([gds_tmp gds_append])

    for arg in $2; do
        gds_tmp=`echo $arg | cut -c1-2`
        gds_append=1
        AS_IF([test "$gds_tmp" = "-I" || test "$gds_tmp" = "-L" || test "$gds_tmp" = "-l"],
              [for val in ${$1}; do
                   AS_IF([test "x$val" = "x$arg"], [gds_append=0])
               done])
        AS_IF([test "$gds_append" = "1"],
...
GDS_VAR_SCOPE_PUSH([gds_prog gds_file gds_dir gds_sentinel])

gds_prog=$1

IFS_SAVE=$IFS
IFS="$PATH_SEPARATOR"
for gds_dir in $PATH; do
    if test -x "$gds_dir/$gds_prog"; then
        $2="$gds_dir/$gds_prog"
        break
    fi
done
...
# Declare some variables; use GDS_VAR_SCOPE_END to ensure that they
# are cleaned up / undefined.
AC_DEFUN([GDS_VAR_SCOPE_PUSH],[

    # Is the private index set?  If not, set it.
    if test "x$gds_scope_index" = "x"; then
        gds_scope_index=1
    fi
    ...
    for gds_var in $1; do
        gds_str="gds_str=\"\$$gds_var\""
        eval $gds_str

        if test "x$gds_str" != "x"; then
            AC_MSG_WARN([Found configure shell variable clash!])
            AC_MSG_WARN([[GDS_VAR_SCOPE_PUSH] called on "$gds_var",])
            AC_MSG_WARN([but it is already defined with value "$gds_str"])
    ...
    gds_str="gds_scope_$gds_scope_index=\"$1\""
    eval $gds_str
    unset gds_str

    env | grep gds_scope
    gds_scope_index=`expr $gds_scope_index + 1`
])dnl

# Unset a bunch of variables that were previously set
AC_DEFUN([GDS_VAR_SCOPE_POP],[
    # Unwind the index
    gds_scope_index=`expr $gds_scope_index - 1`
    gds_scope_test=`expr $gds_scope_index \> 0`
    if test "$gds_scope_test" = "0"; then
        AC_MSG_WARN([[GDS_VAR_SCOPE_POP] popped too many GDS configure scopes.])
        AC_MSG_WARN([This usually indicates an error in configure.])
        AC_MSG_ERROR([Cannot continue])
    fi

    # Get the variable names from that index
    gds_str="gds_str=\"\$gds_scope_$gds_scope_index\""
    eval $gds_str

    # Iterate over all the variables and unset them all
    for gds_var in $gds_str; do
        unset $gds_var
    done
])dnl
...
#
# GDS_WITH_OPTION_MIN_MAX_VALUE(NAME,DEFAULT_VALUE,LOWER_BOUND,UPPER_BOUND)
# Defines a variable GDS_MAX_xxx, with "xxx" being specified as parameter $1 as "variable_name".
...
#
AC_DEFUN([GDS_WITH_OPTION_MIN_MAX_VALUE], [
    max_value=[$2]
    ...
    AC_DEFINE_UNQUOTED([GDS_MAX_]m4_toupper($1), $max_value,
                       [Maximum length of ]m4_translit($1, [_], [ ])[s (default is $2)])
    [GDS_MAX_]m4_toupper($1)=$max_value
    AC_SUBST([GDS_MAX_]m4_toupper($1))
])dnl
...
# Usage: GDS_COMPUTE_MAX_VALUE(number_bytes, variable_to_set, action if overflow)
# Compute maximum value of datatype of
# number_bytes, setting the result in the second argument.  Assumes a
# signed datatype.
AC_DEFUN([GDS_COMPUTE_MAX_VALUE], [
    ...
    gds_num_bits=`expr $1 \* 8 - 1`
    newval=1
    value=1
    overflow=0

    while test $gds_num_bits -ne 0 ; do
        newval=`expr $value \* 2`
        if test 0 -eq `expr $newval \< 0` ; then
            # if the new value is not negative, next iteration...
            value=$newval
            gds_num_bits=`expr $gds_num_bits - 1`
            ...
            # below...)
            if test 0 -eq $gds_num_bits ; then
                value=`expr $value - 1`
            fi
        else
            ...
            if test 0 -eq `expr $newval \< 0` ; then
                ...
                # gds_num_bits is 1, we didn't actually overflow.
                # Otherwise, we overflowed.
                if test 1 -ne $gds_num_bits ; then
                    overflow=1
                fi
            else
