...
AC_DEFUN([GDS_CONFIG_SUBDIR],[
GDS_VAR_SCOPE_PUSH([subdir_parent sub_configure subdir_dir subdir_srcdir subdir_cache_file subdir_args subdir_dots total_dir dir_part temp])
...
if test "$subdir_dir" != ":" && test -d $srcdir/$subdir_dir; then
    AC_MSG_NOTICE([GDS configuring in $subdir_dir])
