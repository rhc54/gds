...
# Minimum versions
my $gds_automake_version = "1.12.2";
my $gds_autoconf_version = "2.69";
my $gds_libtool_version = "2.4.2";

# Search paths
my $gds_autoconf_search = "autoconf";
my $gds_automake_search = "automake";
my $gds_libtoolize_search = "libtoolize;glibtoolize";
...
sub ignored {
    ...
    # If this directory does not have .gds_ignore, or if it has a
    # .gds_unignore that has my username in it, then add it to the
    # list of components.
    my $ignored = 0;
    ...
    if (-f "$dir/.gds_unignore") {
        open(UNIGNORE, "$dir/.gds_unignore") ||
            my_die "Can't open $dir/.gds_unignore file";
        my $unignore;
        $unignore .= $_
            while (<UNIGNORE>);
        close(UNIGNORE);
    ...
    if (exists($exclude_list->{$framework}) &&
        ...
        if (-d $dir) {
            ...
            foreach my $d (readdir(DIR)) {
                ...
                verbose "--- Found gds / $framework / $d component: src/mca/$framework/$d\n";
                ...
                # Check ignore status
                if (ignored("$dir/$d")) {
                    verbose "    => Ignored (found .gds_ignore file)\n";
                } else {
...
sub mca_generate_framework_header(\$\@) {
    ...
    foreach my $framework (@frameworks) {
        ...
        if ($framework ne "common") {
            my $framework_name = "gds_${framework}_base_framework";
            $framework_array_output .= "    &$framework_name,\n";
            $framework_decl_output .= "extern gds_mca_base_framework_t $framework_name;\n";
        }
    }

    my $ifdef_string = uc "gds_FRAMEWORKS_H";
...
#include <gds/mca/base/mca_base_framework.h>

$framework_decl_output
static gds_mca_base_framework_t *gds_frameworks[] = {
...
dnl Frameworks in the gds project and their corresponding directories
m4_define([mca_gds_framework_list], [$frameworks_comma])
...
";
    ...
    foreach my $f (@mykeys) {
        ...
        $m4 .= "dnl Components in the gds / $f framework
m4_define([mca_gds_${f}_m4_config_component_list], [$m4_config_component_list])
m4_define([mca_gds_${f}_no_config_component_list], [$no_config_component_list])
...
tools:
    ...
    GNU Autoconf: $gds_autoconf_version
    GNU Automake: $gds_automake_version
    GNU Libtool: $gds_libtool_version
=================================================================\n";
    my_exit(1);
}
...
my $project_name_long = "GDS";
my $project_name_short = "GDS";
...
# Verify that we're in the GDS root directory by checking for a token file.

my_die "Not at the root directory of an GDS source tree"
    if (! -f "config/gds_mca.m4");

my_die "autogen.pl has been invoked in the source tree of a GDS distribution tarball; aborting...
...
my $step = 1;
verbose "GDS autogen (buckle up!)

$step. Checking tool versions\n\n";

# Check the autotools revision levels
&find_and_check("autoconf", $gds_autoconf_search, $gds_autoconf_version);
&find_and_check("libtool", $gds_libtoolize_search, $gds_libtool_version);
&find_and_check("automake", $gds_automake_search, $gds_automake_version);
...
# Figure out if we're at the top level of the GDS tree or not.
if (! (-f "VERSION" && -f "configure.ac" && -f $topdir_file)) {
    print("\n\nYou must run this script from the top-level directory of the GDS tree.\n\n");
    my_exit(1);
}
...
verbose "
================================================
GDS autogen: completed successfully.  w00t!
================================================\n\n";
