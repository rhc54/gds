...
static const char *path_sep = GDS_PATH_SEP;

char *gds_os_path(bool relative, ...)
{
    va_list ap;
    char *element, *path;
    ...
    if (total_length > GDS_PATH_MAX) {  /* path length is too long - reject it */
    	return(NULL);
    }
    ...
    va_end(ap);
    return gds_make_filename_os_friendly(path);
}
