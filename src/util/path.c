...
bool gds_path_is_absolute( const char *path )
{
    if( GDS_PATH_SEP[0] == *path ) {
        return true;
    }
    return false;
}
 ...
 */
char *gds_path_find(char *fname, char **pathv, int mode, char **envv)
{
    char *fullpath;
    char *delimit;
    char *env;
    ...
    if( gds_path_is_absolute(fname) ) {
        return gds_path_access(fname, NULL, mode);
    }
    ...
    while (pathv[i] && NULL == fullpath) {
        ...
        if ('$' == *pathv[i]) {
            delimit = strchr(pathv[i], GDS_PATH_SEP[0]);
            if (delimit) {
                *delimit = '\0';
            }
            env = list_env_get(pathv[i]+1, envv);
            if (delimit) {
                *delimit = GDS_PATH_SEP[0];
            }
            if (NULL != env) {
                if (!delimit) {
                    fullpath = gds_path_access(fname, env, mode);
                } else {
                    ...
                    strcpy(pfix, env);
                    strcat(pfix, delimit);
                    fullpath = gds_path_access(fname, pfix, mode);
                    free(pfix);
                }
            }
        }
        else {
            fullpath = gds_path_access(fname, pathv[i], mode);
        }
        i++;
    }
    return gds_make_filename_os_friendly(fullpath);
}

/*
 * Locates a file with certain permissions from a list of search paths
 */
char *gds_path_findv(char *fname, int mode, char **envv, char *wrkdir)
{
    char **dirv;
    char *fullpath;
    char *path;
    ...
    if (!found_dot && NULL != wrkdir) {
        gds_argv_append(&dirc, &dirv, wrkdir);
    }

    if(NULL == dirv)
        return NULL;
    fullpath = gds_path_find(fname, dirv, mode, envv);
    gds_argv_free(dirv);
    return fullpath;
}
 ...
 *      -NULL Failure
 */
char *gds_path_access(char *fname, char *path, int mode)
{
    char *fullpath = NULL;
    struct stat buf;
    ...
    if (NULL == path) {
        fullpath = gds_os_path(false, fname, NULL);
    } else {
        fullpath = gds_os_path(false, path, fname, NULL);
    }
    if (NULL == fullpath)
        return NULL;
...
static void path_env_load(char *path, int *pargc, char ***pargv)
{
    ...
    while ('\0' != *path) {
        ...
        if (p != path) {
            saved = *p;
            *p = '\0';
            gds_argv_append(pargc, pargv, path);
            *p = saved;
            path = p;
        }
 ...
 * will be returned.
 */
char* gds_find_absolute_path( char* app_name )
{
    char* abs_app_name;
    char cwd[GDS_PATH_MAX], *pcwd;

    if( gds_path_is_absolute(app_name) ) { /* already absolute path */
        abs_app_name = app_name;
    } else if ( '.' == app_name[0] ||
               NULL != strchr(app_name, GDS_PATH_SEP[0])) {
        /* the app is in the current directory or below it */
        pcwd = getcwd( cwd, GDS_PATH_MAX );
        if( NULL == pcwd ) {
            /* too bad there is no way we can get the app absolute name */
            return NULL;
        }
        abs_app_name = gds_os_path( false, pcwd, app_name, NULL );
    } else {
        /* Otherwise try to search for the application in the PATH ... */
        abs_app_name = gds_path_findv( app_name, X_OK, NULL, NULL );
    }

    if( NULL != abs_app_name ) {
        char* resolved_path = (char*)malloc(GDS_PATH_MAX);
...
bool gds_path_nfs(char *fname, char **ret_fstype)
{
    int i;
    int fsrc = -1;
    int vfsrc = -1;
...
#endif
    /*
     * Be sure to update the test (test/util/gds_path_nfs.c)
...
again:
    ...
    if (-1 == fsrc && -1 == vfsrc) {
        ...
        GDS_OUTPUT_VERBOSE((10, 0, "gds_path_nfs: stat(v)fs on file:%s failed errno:%d directory:%s\n",
                             fname, errno, file));
        if (EPERM == errno) {
            free(file);
            if ( NULL != ret_fstype ) {
...
#    endif
  ...
  found:
    ...
    if (AUTOFS_SUPER_MAGIC == fs_types[i].f_fsid) {
        char *fs_type = gds_check_mtab(fname);
        int x;
        if (NULL != fs_type) {
            for (x = 0; x < FS_TYPES_NUM; x++) {
                ...
                if (0 == strcasecmp(fs_types[x].f_fsname, fs_type)) {
                    GDS_OUTPUT_VERBOSE((10, 0, "gds_path_nfs: file:%s on fs:%s\n", fname, fs_type));
                    free(fs_type);
                    if ( NULL != ret_fstype ) {
             ...
             uint64_t *out_avail)
{
    ...
    *out_avail = 0;
    ...
    if (-1 == rc) {
        GDS_OUTPUT_VERBOSE((10, 2, "gds_path_df: stat(v)fs on "
    ...
    *out_avail = buf.f_bsize * ((int)buf.f_bavail < 0 ? 0 : buf.f_bavail);
    ...
    GDS_OUTPUT_VERBOSE((10, 2, "gds_path_df: stat(v)fs states "
