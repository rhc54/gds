...
#if (GDS_ALIGNMENT_LONG == 8)
#define GDS_CRC_WORD_MASK_ 0x7
#elif (GDS_ALIGNMENT_LONG == 4)
#define GDS_CRC_WORD_MASK_ 0x3
#else
#define GDS_CRC_WORD_MASK_ 0xFFFF
#endif


#define WORDALIGNED(v) \
    (((intptr_t)v & GDS_CRC_WORD_MASK_) ? false : true)
...
static bool _gds_crc_table_initialized = false;
static unsigned int _gds_crc_table[256];
...
void gds_initialize_crc_table(void)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        _gds_crc_table[i] = crc_accum;
    }

    /* set global bool to true to do this work once! */
    _gds_crc_table_initialized = true;
    return;
}

unsigned int gds_bcopy_uicrc_partial(
    const void *  source,
    void *  destination,
    size_t copylen,
    size_t crclen,
        ...
        while (copylen >= sizeof(unsigned int)) {
            ...
            *dst++ = tmp;
            ...
            for (j = 0; j < (int)sizeof(unsigned int); j++) {
                ...
                partial_crc = (partial_crc << 8) ^ _gds_crc_table[i];
            }
...
unsigned int gds_uicrc_partial(
