 ...
 * @file
 *
 * The "show help" subsystem (SHS) in GDS is intended to help the
 ...
 *
 * The function gds_show_help() is used to find a help message and
 ...
 * for that file in $pkgdatadir (typically $prefix/share/gds).
 *
 ...
 * gds_show_help() takes a variable legnth argument list that are
 ...
 * component may install its own helpfile in GDS's $pkgdatadir,
 * and therefore the component can invoke gds_show_help() to display
...
#ifndef GDS_SHOW_HELP_H
#define GDS_SHOW_HELP_H
 ...
 * (typically $prefix/share/gds), and looks up the message
 * based on the topic, and displays it.  If want_error_header is
 * true, a header and footer of asterisks are also displayed.
 */
typedef int (*gds_show_help_fn_t)(const char *filename, const char *topic,
                                   bool want_error_header, ...);
extern gds_show_help_fn_t gds_show_help;

/**
 * This function does the same thing as gds_show_help(), but accepts
 * a va_list form of varargs.
 */
typedef int (*gds_show_vhelp_fn_t)(const char *filename, const char *topic,
                                    bool want_error_header, va_list ap);
extern gds_show_vhelp_fn_t gds_show_vhelp;

/**
 * This function does the same thing as gds_show_help(), but returns
 * its output in a string (that must be freed by the caller).
 */
char* gds_show_help_string(const char *filename,
                                          const char *topic,
                                          bool want_error_header, ...);

/**
 * This function does the same thing as gds_show_help_string(), but
 * accepts a va_list form of varargs.
 */
char* gds_show_help_vstring(const char *filename,
                                          const char *topic,
                                          bool want_error_header, va_list ap);
