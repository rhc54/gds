...
/** @file
 * GDS output stream facility.
 *
 * The GDS output stream facility is used to send output from the GDS
 ...
 * gds_output_open() function call, and then sent output via
 * gds_output_verbose(), GDS_OUTPUT(), and gds_output().  Streams are
 * closed with gds_output_close().
 *
 ...
 * - file
 *
 * Which outputs to use are specified during gds_output_open().
 *
 ...
 * not be able to write to a file for output.  See gds_output() for
 * details on what happens in these cases.
 *
 * gds_output_open() returns an integer handle that is used in
 * successive calls to GDS_OUTPUT() and gds_output() to send output to
 * the stream.
 *
 * The default "verbose" stream is opened after invoking
 * gds_output_init() (and closed after invoking
 * gds_output_finalize()).  This stream outputs to stderr only, and
...
#ifndef GDS_OUTPUT_H_
#define GDS_OUTPUT_H_
 ...
 * output (both from gds_output and stdout/stderr of processes) to
 * syslog:
 *
 * GDS_OUTPUT_REDIRECT - set to "syslog" to redirect to syslog. Other
 *                        options may someday be supported
 * GDS_OUTPUT_SYSLOG_PRI - set to "info", "error", or "warn" to have
 *                        output sent to syslog at that priority
 * GDS_OUTPUT_SYSLOG_IDENT - a string identifier for the log
 *
 ...
 */
extern bool gds_output_redirected_to_syslog;
extern int gds_output_redirected_syslog_pri;

/**
 * \class gds_output_stream_t
 *
 * Structure used to request the opening of a GDS output stream.  A
 * pointer to this structure is passed to gds_output_open() to tell
 * the gds_output subsystem where to send output for a given stream.
 ...
 * destinations through the gds_output facility.
 *
 ...
 * from gds_output_open().
 */
struct gds_output_stream_t {
    /** Class parent */
    gds_object_t super;
     ...
     * gds_output_verbose() function.  Verbose levels are ignored in
     * GDS_OUTPUT() and gds_output().
     *
    ...
    /**
     * When gds_output_stream_t::lds_want_syslog is true, this field is
     ...
     *
     * If a NULL value is given, the string "gds" is used.
     */
     ...
     * to an internal structure in the call to gds_output_open()!
     */
    char *lds_prefix;
     ...
     * the stream unless GDS was configured with --enable-debug.
     */
    ...
    bool lds_want_file;
    /**
     * When gds_output_stream_t::lds_want_file is true, this field
     ...
     * prefix set by gds_output_set_output_file_info() (e.g.,
     ...
     * exist when gds_output_open() is invoked.  See gds_output()
     ...
     * Convenience typedef
     */
    typedef struct gds_output_stream_t gds_output_stream_t;
     ...
     * stream.
     */
    void gds_output_finalize(void);

    /**
     * Opens an output stream.
     *
     * @param lds A pointer to gds_output_stream_t describing what the
     ...
     * using it in successive calls to GDS_OUTPUT(), gds_output(),
     * gds_output_switch(), and gds_output_close().
     *
     ...
     * unspecified manner.
     *
     * Be sure to see gds_output() for a description of what happens
     * when open_open() / gds_output() is directed to send output to a
     * file but the process session directory does not yet exist.
     */
    int gds_output_open(gds_output_stream_t *lds);
     ...
     */
    int gds_output_reopen(int output_id, gds_output_stream_t *lds);
     ...
     * the gds_output subsystem to intercept and discard any output sent
     * to the stream via GDS_OUTPUT() or gds_output() until the output
     * is re-enabled.
     */
    bool gds_output_switch(int output_id, bool enable);
     ...
     */
    void gds_output_reopen_all(void);
     ...
     */
    void gds_output_close(int output_id);

    /**
     * Main function to send output to a stream.
     *
     * @param output_id Stream id returned from gds_output_open().
     ...
     * gds_output_verbose() and gds_output_error()).
     *
     ...
     * session directory does not exist when gds_output() is invoked,
     * the output will be discarded!  Once the session directory is
     * created, gds_output() will automatically create the file and
     * writing to it.
     */
    void gds_output(int output_id, const char *format, ...) __gds_attribute_format__(__printf__, 2, 3);
     ...
     * function will effectively invoke gds_output to send the output to
     * the stream.
     *
     * @see gds_output_set_verbosity()
     */
    void gds_output_verbose(int verbose_level, int output_id,
                                           const char *format, ...) __gds_attribute_format__(__printf__, 3, 4);

   /**
    * Same as gds_output_verbose(), but takes a va_list form of varargs.
    */
    void gds_output_vverbose(int verbose_level, int output_id,
                                            const char *format, va_list ap) __gds_attribute_format__(__printf__, 3, 0);
     ...
     * specified by the format parameter.
     *
     * Exactly the same as gds_output_verbose(), except the output it
     ...
     */
    char *gds_output_string(int verbose_level, int output_id,
                                           const char *format, ...) __gds_attribute_format__(__printf__, 3, 4);

   /**
    * Same as gds_output_string, but accepts a va_list form of varargs.
    */
    char *gds_output_vstring(int verbose_level, int output_id,
                                            const char *format, va_list ap) __gds_attribute_format__(__printf__, 3, 0);
     ...
     * will be used for all future invocations of gds_output_verbose().
     */
    void gds_output_set_verbosity(int output_id, int level);
     ...
     */
    int gds_output_get_verbosity(int output_id);
     ...
     * gds_output_stream_t::lds_want_file is true, the output
     ...
     * after gds_output_set_output_file_info() returns.
     *
     ...
     * to the new directory).  More specifically, the gds_output
     ...
     */
    void gds_output_set_output_file_info(const char *dir,
                                                        const char *prefix,
                                                        char **olddir,
                                                        char **oldprefix);

    /**
     * Same as gds_output_verbose(), but pointer to buffer and size.
     */
    void gds_output_hexdump(int verbose_level, int output_id,
                                           void *ptr, int buflen);
     ...
     * @see gds_output()
     */
#define GDS_OUTPUT(a) gds_output a

    /**
     * Macro for use in sending debugging output to the output
     * streams.  Will be "compiled out" when GDS is configured
     * without --enable-debug.
     *
     * @see gds_output_verbose()
     */
#define GDS_OUTPUT_VERBOSE(a) gds_output_verbose a
#else
    /**
     * Main macro for use in sending debugging output to output streams;
     * will be "compiled out" when GDS is configured without
 ...
 */
GDS_CLASS_DECLARATION(gds_output_stream_t);
