...
#ifndef GDS_ENVIRON_H
#define GDS_ENVIRON_H
 ...
 * later be freed with a call to gds_argv_free().
 *
 ...
 */
char **gds_environ_merge(char **minor, char **major) __gds_attribute_warn_unused_result__;
 ...
 *
 * @retval GDS_ERR_OUT_OF_RESOURCE If internal malloc() fails.
 * @retval GDS_EXISTS If the name already exists in \em env and
 * \em overwrite is false (and therefore the \em value was not
 * saved in \em env)
 * @retval GDS_SUCESS If the value replaced another value or is
 * appended to \em env.
 *
 ...
 *
 * \code
 *   #include "gds/util/gds_environ.h"
 *   gds_setenv("foo", "bar", true, &environ);
 * \endcode
 *
 * NOTE: If you use the real environ, gds_setenv() will turn
 ...
 * \code
 *   char **my_env = NULL;
 *   gds_setenv("foo", "bar", true, &my_env);
 * \endcode
 */
gds_status_t gds_setenv(const char *name, const char *value,
                              bool overwrite, char ***env) __gds_attribute_nonnull__(1);
 ...
 * @param env The environment to use
 *
 * @retval GDS_ERR_OUT_OF_RESOURCE If an internal malloc fails.
 * @retval GDS_ERR_NOT_FOUND If \em name is not found in \em env.
 * @retval GDS_SUCCESS If \em name is found and successfully deleted.
 *
 * If \em name is found in \em env, the string corresponding to
 * that entry is freed and its entry is eliminated from the array.
 */
gds_status_t gds_unsetenv(const char *name, char ***env) __gds_attribute_nonnull__(1);
