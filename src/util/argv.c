 ...
 */
gds_status_t gds_argv_append(int *argc, char ***argv, const char *arg)
{
    gds_status_t rc;

    /* add the new element */
    if (GDS_SUCCESS != (rc = gds_argv_append_nosize(argv, arg))) {
        return rc;
    }

    *argc = gds_argv_count(*argv);

    return GDS_SUCCESS;
}

gds_status_t gds_argv_append_nosize(char ***argv, const char *arg)
{
    int argc;
...
gds_status_t gds_argv_prepend_nosize(char ***argv, const char *arg)
{
    int argc;
    int i;
...
gds_status_t gds_argv_append_unique_nosize(char ***argv, const char *arg, bool overwrite)
{
    int i;
     ...
     */
    if (NULL == *argv) {
        return gds_argv_append_nosize(argv, arg);
    }
 ...
 */
static char **gds_argv_split_inter(const char *src_string, int delimiter,
        int include_empty)
{
  char arg[ARGSIZE];
  char **argv = NULL;
  ...
  while (src_string && *src_string) {
    ...
    if (src_string == p) {
      if (include_empty) {
        arg[0] = '\0';
        if (GDS_SUCCESS != gds_argv_append(&argc, &argv, arg))
          return NULL;
      }
    }
    ...
    /* tail argument, add straight from the original string */
      ...
      if (GDS_SUCCESS != gds_argv_append(&argc, &argv, src_string))
	return NULL;
      src_string = p;
      continue;
    }
	...
	return NULL;
      ...
      if (GDS_SUCCESS != gds_argv_append(&argc, &argv, argtemp)) {
	free(argtemp);
	return NULL;
      }
...
char **gds_argv_split(const char *src_string, int delimiter)
{
    return gds_argv_split_inter(src_string, delimiter, 0);
}

char **gds_argv_split_with_empty(const char *src_string, int delimiter)
{
    return gds_argv_split_inter(src_string, delimiter, 1);
}

/*
 * Return the length of a NULL-terminated argv array.
 */
int gds_argv_count(char **argv)
{
  char **p;
  int i;
 ...
 */
char *gds_argv_join(char **argv, int delimiter)
{
  char **p;
  char *pp;
  char *str;
 ...
 */
char *gds_argv_join_range(char **argv, size_t start, size_t end, int delimiter)
{
    char **p;
    char *pp;
    char *str;
    ...
    if (NULL == argv || NULL == argv[0] || (int)start > gds_argv_count(argv)) {
        return strdup("");
    }
...
char **gds_argv_copy(char **argv)
{
  ...
  while (NULL != *argv) {
    if (GDS_SUCCESS != gds_argv_append(&dupc, &dupv, *argv)) {
      gds_argv_free(dupv);
      return NULL;
    }
...
gds_status_t gds_argv_delete(int *argc, char ***argv, int start, int num_to_delete)
{
    int i;
    int count;
    int suffix_count;
...
gds_status_t gds_argv_insert(char ***target, int start, char **source)
{
    ...
    target_count = gds_argv_count(*target);
    source_count = gds_argv_count(source);
    if (start > target_count) {
        for (i = 0; i < source_count; ++i) {
            gds_argv_append(&target_count, target, source[i]);
        }
    }
...
gds_status_t gds_argv_insert_element(char ***target, int location, char *source)
{
    int i, target_count;
    int suffix_count;
    ...
    if (location > target_count) {
        gds_argv_append(&target_count, target, source);
        return GDS_SUCCESS;
    }
