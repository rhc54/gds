...
#if GDS_ENABLE_DEBUG
                    gds_output(0, "GDS DEBUG WARNING: Got a NULL argument to gds_vasprintf!\n");
#endif
                    len += 5;
                }
                break;
...
int gds_asprintf(char **ptr, const char *fmt, ...)
{
    int length;
    va_list ap;

    va_start(ap, fmt);
    length = gds_vasprintf(ptr, fmt, ap);
    va_end(ap);
...
int gds_vasprintf(char **ptr, const char *fmt, va_list ap)
{
    int length;
    va_list ap2;
...
#if GDS_HAVE_VA_COPY
    va_copy(ap2, ap);
#elif GDS_HAVE_UNDERSCORE_VA_COPY
    __va_copy(ap2, ap);
#else
...
#if GDS_HAVE_VA_COPY || GDS_HAVE_UNDERSCORE_VA_COPY
    va_end(ap2);
#endif  /* GDS_HAVE_VA_COPY || GDS_HAVE_UNDERSCORE_VA_COPY */
...
int gds_snprintf(char *str, size_t size, const char *fmt, ...)
{
    int length;
    va_list ap;

    va_start(ap, fmt);
    length = gds_vsnprintf(str, size, fmt, ap);
    va_end(ap);
...
int gds_vsnprintf(char *str, size_t size, const char *fmt, va_list ap)
{
    int length;
    char *buf;

    length = gds_vasprintf(&buf, fmt, ap);
    if (length < 0) {
        return length;
    }
...
int main(int argc, char *argv[])
{
    ...
    length = gds_snprintf(NULL, 0, "this is a string %d", 1004);
    printf("length = %d\n", length);

    puts("test of snprintf to an undersize buffer:");
    length = gds_snprintf(a, sizeof(a), "this is a string %d", 1004);
    printf("string = %s\n", a);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(a));

    puts("test of snprintf to an oversize buffer:");
    length = gds_snprintf(b, sizeof(b), "this is a string %d", 1004);
    printf("string = %s\n", b);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(b));

    puts("test of asprintf:");
    length = gds_asprintf(&s, "this is a string %d", 1004);
    printf("string = %s\n", s);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(s));
