...
static gds_output_stream_t verbose;
 ...
 * Private functions
 */
static void construct(gds_object_t *stream);
static int do_open(int output_id, gds_output_stream_t * lds);
...
#define GDS_OUTPUT_MAX_STREAMS 64
...
/* global state */
bool gds_output_redirected_to_syslog = false;
int gds_output_redirected_syslog_pri = 0;
...
static output_desc_t info[GDS_OUTPUT_MAX_STREAMS];
...
GDS_CLASS_INSTANCE(gds_output_stream_t, gds_object_t, construct, NULL);
...
{
    int i;
    char hostname[GDS_MAXHOSTNAMELEN];
    char *str;
    ...
    str = getenv("GDS_OUTPUT_STDERR_FD");
    if (NULL != str) {
        default_stderr_fd = atoi(str);
    }
    str = getenv("GDS_OUTPUT_REDIRECT");
    if (NULL != str) {
        if (0 == strcasecmp(str, "syslog")) {
            gds_output_redirected_to_syslog = true;
        }
    }
    str = getenv("GDS_OUTPUT_SYSLOG_PRI");
    if (NULL != str) {
        if (0 == strcasecmp(str, "info")) {
            gds_output_redirected_syslog_pri = LOG_INFO;
        } else if (0 == strcasecmp(str, "error")) {
            gds_output_redirected_syslog_pri = LOG_ERR;
        } else if (0 == strcasecmp(str, "warn")) {
            gds_output_redirected_syslog_pri = LOG_WARNING;
        } else {
            gds_output_redirected_syslog_pri = LOG_ERR;
        }
    } else {
    ...
    str = getenv("GDS_OUTPUT_SYSLOG_IDENT");
    if (NULL != str) {
        redirect_syslog_ident = strdup(str);
    }

    GDS_CONSTRUCT(&verbose, gds_output_stream_t);
    if (gds_output_redirected_to_syslog) {
        verbose.lds_want_syslog = true;
        verbose.lds_syslog_priority = gds_output_redirected_syslog_pri;
        if (NULL != str) {
    ...
    if (0 > asprintf(&verbose.lds_prefix, "[%s:%05d] ", hostname, getpid())) {
        return GDS_ERR_NOMEM;
    }

    for (i = 0; i < GDS_OUTPUT_MAX_STREAMS; ++i) {
        info[i].ldi_used = false;
        info[i].ldi_enabled = false;

        info[i].ldi_syslog = gds_output_redirected_to_syslog;
    ...
    output_dir = strdup(gds_tmp_directory());

    /* Open the default verbose stream */
    verbose_stream = gds_output_open(&verbose);
    return true;
}
...
bool gds_output_switch(int output_id, bool enable)
{
    ...
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS) {
...
#if defined(HAVE_SYSLOG)
        if (i >= GDS_OUTPUT_MAX_STREAMS && syslog_opened) {
            closelog();
        }
#endif
    }
 ...
 */
void gds_output(int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS) {
        va_list arglist;
 ...
 */
void gds_output_verbose(int level, int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS &&
 ...
 * Send a message to a stream if the verbose level is high enough
 */
void gds_output_vverbose(int level, int output_id, const char *format,
                          va_list arglist)
{
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS &&
        info[output_id].ldi_verbose_level >= level) {
 ...
 */
char *gds_output_string(int level, int output_id, const char *format, ...)
{
    int rc;
    char *ret = NULL;
 ...
 * Send a message to a string if the verbose level is high enough
 */
char *gds_output_vstring(int level, int output_id, const char *format,
                          va_list arglist)
{
    int rc;
    char *ret = NULL;
                         ...
                         void *ptr, int buflen)
{
    ...
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS &&
        ...
        gds_output_verbose(verbose_level, output_id, "dump data at %p %d bytes\n", ptr, buflen);
        for (i = 0; i < buflen; i += 16) {
            ...
            if (ret < 0)
            return;
            gds_output_verbose(verbose_level, output_id, "%s", out_buf);
        }
    }
}
...
void gds_output_finalize(void)
{
    if (initialized) {
        if (verbose_stream != -1) {
            gds_output_close(verbose_stream);
        }
        free(verbose.lds_prefix);
        verbose_stream = -1;
 ...
 * Constructor
 */
static void construct(gds_object_t *obj)
{
    gds_output_stream_t *stream = (gds_output_stream_t*) obj;
...
static int do_open(int output_id, gds_output_stream_t * lds)
{
    ...
    if (gds_output_redirected_to_syslog) {
        ...
        info[i].ldi_syslog_priority = gds_output_redirected_syslog_pri;
...
static int open_file(int i)
{
     ...
     * to open it twice
     */
    for (n=0; n < GDS_OUTPUT_MAX_STREAMS; n++) {
        if (i == n) {
            continue;
        }
        if (!info[n].ldi_used) {
    ...
    if (NULL != output_dir) {
        filename = (char *) malloc(GDS_PATH_MAX);
        if (NULL == filename) {
            return GDS_ERR_OUT_OF_RESOURCE;
        }
        strncpy(filename, output_dir, GDS_PATH_MAX);
        strcat(filename, "/");
...
static int output(int output_id, const char *format, va_list arglist)
{
    ...
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS &&
        ...
        /* Make the strings */
        if (GDS_SUCCESS != (rc = make_string(&str, ldi, format, arglist))) {
            return rc;
        }
         ...
         * gds_output.h docs about this!). */

        if (ldi->ldi_file) {
            if (ldi->ldi_fd == -1) {
                if (GDS_SUCCESS != open_file(output_id)) {
...
{
    if (output_id >= 0 && output_id < GDS_OUTPUT_MAX_STREAMS && info[output_id].ldi_used) {
