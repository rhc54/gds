 ...
 */
static inline char* gds_find_last_path_separator( const char* filename, size_t n )
{
...
char *gds_basename(const char *filename)
{
    size_t i;
    char *tmp, *ret = NULL;
    const char sep = GDS_PATH_SEP[0];
    ...
    ret = gds_find_last_path_separator( tmp, strlen(tmp) );
    if (NULL == ret) {
        return tmp;
    }
    ret = strdup(ret + 1);
...
char* gds_dirname(const char* filename)
{
#if defined(HAVE_DIRNAME) || GDS_HAVE_DIRNAME
    char* safe_tmp = strdup(filename), *result;
    result = strdup(dirname(safe_tmp));
    free(safe_tmp);
    return result;
#else
    const char* p = gds_find_last_path_separator(filename, strlen(filename));
...
#endif
                ret[p - filename] = '\0';
                return gds_make_filename_os_friendly(ret);
            }
            break;  /* return the duplicate of "." */
        }
    }
    return strdup(".");
#endif  /* defined(HAVE_DIRNAME) || GDS_HAVE_DIRNAME */
}
