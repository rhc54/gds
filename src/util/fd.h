...
#ifndef GDS_UTIL_FD_H_
#define GDS_UTIL_FD_H_
 ...
 *
 * @returns GDS_SUCCESS upon success.
 * @returns GDS_ERR_TIMEOUT if the fd closes before reading the full amount.
 * @returns GDS_ERR_IN_ERRNO otherwise.
 *
 * Loop over reading from the fd until len bytes are read or an error
 * occurs.  EAGAIN and EINTR are transparently handled.
 */
gds_status_t gds_fd_read(int fd, int len, void *buffer);
 ...
 * occurs.  EAGAIN and EINTR are transparently handled.
 */
gds_status_t gds_fd_write(int fd, int len, const void *buffer);
 ...
 * @param fd File descriptor
 *
 * @returns GDS_SUCCESS upon success (or if the system does not
 ...
 */
gds_status_t gds_fd_set_cloexec(int fd);
