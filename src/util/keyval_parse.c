...
int gds_util_keyval_parse_lineno = 0;

static const char *keyval_filename;
static gds_keyval_parse_fn_t keyval_callback;
...
int gds_util_keyval_parse_init(void)
{
    return GDS_SUCCESS;
}
...
int
gds_util_keyval_parse_finalize(void)
{
...
int
gds_util_keyval_parse(const char *filename,
                       gds_keyval_parse_fn_t callback)
{
    int val;
    int ret = GDS_SUCCESS;;
    ...
    /* Open the gds */
    gds_util_keyval_yyin = fopen(keyval_filename, "r");
    if (NULL == gds_util_keyval_yyin) {
        ret = GDS_ERR_NOT_FOUND;
        goto cleanup;
    }

    gds_util_keyval_parse_done = false;
    gds_util_keyval_yynewlines = 1;
    gds_util_keyval_init_buffer(gds_util_keyval_yyin);
    while (!gds_util_keyval_parse_done) {
        val = gds_util_keyval_yylex();
        switch (val) {
        case GDS_UTIL_KEYVAL_PARSE_DONE:
            /* This will also set gds_util_keyval_parse_done to true, so just
               break here */
            break;

        case GDS_UTIL_KEYVAL_PARSE_NEWLINE:
            /* blank line!  ignore it */
            break;

        case GDS_UTIL_KEYVAL_PARSE_SINGLE_WORD:
            parse_line();
            break;

        case GDS_UTIL_KEYVAL_PARSE_MCAVAR:
        case GDS_UTIL_KEYVAL_PARSE_ENVVAR:
        case GDS_UTIL_KEYVAL_PARSE_ENVEQL:
            parse_line_new(val);
            break;
    ...
    fclose(gds_util_keyval_yyin);
    gds_util_keyval_yylex_destroy ();
...
static int parse_line(void)
{
    ...
    gds_util_keyval_parse_lineno = gds_util_keyval_yylineno;

    /* Save the name name */
    if (key_buffer_len < strlen(gds_util_keyval_yytext) + 1) {
        char *tmp;
        key_buffer_len = strlen(gds_util_keyval_yytext) + 1;
    ...
    strncpy(key_buffer, gds_util_keyval_yytext, key_buffer_len);

    /* The first thing we have to see is an "=" */

    val = gds_util_keyval_yylex();
    if (gds_util_keyval_parse_done || GDS_UTIL_KEYVAL_PARSE_EQUAL != val) {
        parse_error(2);
        return GDS_ERROR;
    }
    ...
    val = gds_util_keyval_yylex();
    if (GDS_UTIL_KEYVAL_PARSE_SINGLE_WORD == val ||
        GDS_UTIL_KEYVAL_PARSE_VALUE == val) {
        keyval_callback(key_buffer, gds_util_keyval_yytext);

        /* Now we need to see the newline */

        val = gds_util_keyval_yylex();
        if (GDS_UTIL_KEYVAL_PARSE_NEWLINE == val ||
            GDS_UTIL_KEYVAL_PARSE_DONE == val) {
            return GDS_SUCCESS;
        }
    }
    ...
    else if (GDS_UTIL_KEYVAL_PARSE_DONE == val ||
             GDS_UTIL_KEYVAL_PARSE_NEWLINE == val) {
...
static void parse_error(int num)
{
    ...
    gds_output(0, "keyval parser: error %d reading file %s at line %d:\n  %s\n",
                num, keyval_filename, gds_util_keyval_yynewlines, gds_util_keyval_yytext);
}

int gds_util_keyval_save_internal_envars(gds_keyval_parse_fn_t callback)
{
