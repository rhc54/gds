 ...
 * Private functions
 */
static void find_dyn_components(const char *path, gds_mca_base_framework_t *framework,
                                const char **names, bool include_mode);

#endif /* GDS_HAVE_GDL_SUPPORT */

static int component_find_check (gds_mca_base_framework_t *framework, char **requested_component_names);

/*
 * Dummy structure for casting for open_only logic
 */
struct gds_mca_base_open_only_dummy_component_t {
    /** MCA base component */
    gds_mca_base_component_t version;
    /** MCA base data */
    gds_mca_base_component_data_t data;
};
typedef struct gds_mca_base_open_only_dummy_component_t gds_mca_base_open_only_dummy_component_t;
 ...
 *
 * Return one consolidated array of (gds_mca_base_component_t*) pointing to all
...
{
    const gds_mca_base_component_t **static_components = framework->framework_static_components;
    char **requested_component_names = NULL;
    gds_mca_base_component_list_item_t *cli;
    bool include_mode = true;
    int ret;

    gds_output_verbose (GDS_MCA_BASE_VERBOSE_COMPONENT, framework->framework_output,
                         "mca: base: component_find: searching %s for %s components",
                         directory, framework->framework_name);

    if (!ignore_requested) {
        ret = gds_mca_base_component_parse_requested (framework->framework_selection, &include_mode,
    ...
    if (static_components) {
        for (int i = 0 ; NULL != static_components[i]; ++i) {
            if ( use_component(include_mode,
                               (const char**)requested_component_names,
                               static_components[i]->gds_mca_component_name) ) {
                cli = GDS_NEW(gds_mca_base_component_list_item_t);
                if (NULL == cli) {
                    ret = GDS_ERR_OUT_OF_RESOURCE;
                    goto component_find_out;
                }
                cli->cli_component = static_components[i];
                gds_list_append(&framework->framework_components, (gds_list_item_t *) cli);
            }
        }
    }
    ...
    if (open_dso_components && !gds_mca_base_component_disable_dlopen) {
        find_dyn_components(directory, framework, (const char**)requested_component_names,
                            include_mode);
    } else {
        gds_output_verbose (GDS_MCA_BASE_VERBOSE_INFO, 0,
                            "gds:mca: base: component_find: dso loading for %s MCA components disabled",
...
component_find_out:
    ...
    if (NULL != requested_component_names) {
        gds_argv_free(requested_component_names);
    }
...
int gds_mca_base_components_filter (gds_mca_base_framework_t *framework, uint32_t filter_flags)
{
    ...
    GDS_LIST_FOREACH_SAFE(cli, next, components, gds_mca_base_component_list_item_t) {
        ...
        gds_mca_base_open_only_dummy_component_t *dummy =
            (gds_mca_base_open_only_dummy_component_t *) cli->cli_component;

        can_use = use_component(include_mode, (const char **) requested_component_names,
                                cli->cli_component->gds_mca_component_name);

        if (!can_use || (filter_flags & dummy->data.param_field) != filter_flags) {
            if (can_use && (filter_flags & GDS_MCA_BASE_METADATA_PARAM_CHECKPOINT) &&
                !(GDS_MCA_BASE_METADATA_PARAM_CHECKPOINT & dummy->data.param_field)) {
                gds_output_verbose(GDS_MCA_BASE_VERBOSE_COMPONENT, output_id,
                                    "gds:mca: base: components_filter: "
            ...
            GDS_RELEASE(cli);
        } else if (filter_flags & GDS_MCA_BASE_METADATA_PARAM_CHECKPOINT) {
            gds_output_verbose(GDS_MCA_BASE_VERBOSE_COMPONENT, output_id,
                                "gds:mca: base: components_filter: "
                                ...
                                const char **names, bool include_mode)
{
    ...
    if (NULL != path) {
        ret = gds_mca_base_component_repository_add(path);
        if (GDS_SUCCESS != ret) {
            return;
        }
    }

    ret = gds_mca_base_component_repository_get_components(framework, &dy_components);
    if (GDS_SUCCESS != ret) {
        return;
    }

    /* Iterate through the repository and find components that can be included */
    GDS_LIST_FOREACH(ri, dy_components, gds_mca_base_component_repository_item_t) {
        if (use_component(include_mode, names, ri->ri_name)) {
            gds_mca_base_component_repository_open(framework, ri);
        }
    }
}
...
static int component_find_check (gds_mca_base_framework_t *framework, char **requested_component_names)
{
    ...
    for (int i = 0; NULL != requested_component_names[i]; ++i) {
        ...
        GDS_LIST_FOREACH(cli, components, gds_mca_base_component_list_item_t) {
        ...
        if (!found) {
            ...
            gds_show_help("help-mca-base.txt",
                                        ...
                                        char ***requested_component_names)
{
    ...
    *requested_component_names = gds_argv_split(requested, ',');
