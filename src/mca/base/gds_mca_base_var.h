 ...
 *
 * Note that there is a second header file (gds_mca_base_vari.h)
 ...
 * itself; it should not be required by any other GDS entities.
 */

#ifndef GDS_MCA_BASE_VAR_H
#define GDS_MCA_BASE_VAR_H
...
#include "src/mca/base/gds_mca_base_var_group.h"
...
typedef enum {
    /** The default value */
    GDS_MCA_BASE_VAR_SOURCE_DEFAULT,
    /** The value came from the command line */
    GDS_MCA_BASE_VAR_SOURCE_COMMAND_LINE,
    /** The value came from the environment */
    GDS_MCA_BASE_VAR_SOURCE_ENV,
    /** The value came from a file */
    GDS_MCA_BASE_VAR_SOURCE_FILE,
    /** The value came a "set" API call */
    GDS_MCA_BASE_VAR_SOURCE_SET,
    /** The value came from the override file */
    GDS_MCA_BASE_VAR_SOURCE_OVERRIDE,

    /** Maximum source type */
    GDS_MCA_BASE_VAR_SOURCE_MAX
} gds_mca_base_var_source_t;
...
typedef enum {
    ...
    GDS_MCA_BASE_VAR_SCOPE_CONSTANT,
    /** Setting the READONLY flag means that the gds_mca_base_var_set()
    ...
    /** The value of this variable must be set to a consistent value
        within a group */
    GDS_MCA_BASE_VAR_SCOPE_GROUP,
    /** The value of this variable must be set to the same value
        within a group */
    GDS_MCA_BASE_VAR_SCOPE_GROUP_EQ,
    /** The value of this variable must be set to a consistent value
        for all processes */
    GDS_MCA_BASE_VAR_SCOPE_ALL,
    /** The value of this variable must be set to the same value
        for all processes */
    GDS_MCA_BASE_VAR_SCOPE_ALL_EQ,
    GDS_MCA_BASE_VAR_SCOPE_MAX
} gds_mca_base_var_scope_t;
...
typedef enum {
    ...
    GDS_INFO_LVL_9,
    GDS_INFO_LVL_MAX
} gds_mca_base_var_info_lvl_t;

typedef enum {
    GDS_MCA_BASE_VAR_SYN_FLAG_DEPRECATED = 0x0001,
    GDS_MCA_BASE_VAR_SYN_FLAG_INTERNAL   = 0x0002
} gds_mca_base_var_syn_flag_t;

typedef enum {
    /** Variable is internal (hidden from *_info/MPIT) */
    GDS_MCA_BASE_VAR_FLAG_INTERNAL     = 0x0001,
    /** Variable will always be the default value. Implies
        !MCA_BASE_VAR_FLAG_SETTABLE */
    GDS_MCA_BASE_VAR_FLAG_DEFAULT_ONLY = 0x0002,
    /** Variable can be set with gds_mca_base_var_set() */
    GDS_MCA_BASE_VAR_FLAG_SETTABLE     = 0x0004,
    /** Variable is deprecated */
    GDS_MCA_BASE_VAR_FLAG_DEPRECATED   = 0x0008,
    /** Variable has been overridden */
    GDS_MCA_BASE_VAR_FLAG_OVERRIDE     = 0x0010,
    /** Variable may not be set from a file */
    GDS_MCA_BASE_VAR_FLAG_ENVIRONMENT_ONLY = 0x0020,
    /** Variable should be deregistered when the group is deregistered
        ...
        gds_mca_base_component_var_register(), but can also be set
        manually when you register a variable with
        gds_mca_base_var_register().  Analogous to the
        MCA_BASE_PVAR_FLAG_IWG. */
    GDS_MCA_BASE_VAR_FLAG_DWG          = 0x0040
} gds_mca_base_var_flag_t;
...
typedef union {
    ...
    /** double value */
    double lfval;
} gds_mca_base_var_storage_t;


/**
 * Entry for holding information about an MCA variable.
 */
struct gds_mca_base_var_t {
    /** Allow this to be an GDS OBJ */
    gds_object_t super;

    /** Variable index. This will remain constant until gds_mca_base_var_finalize()
        is called. */
    int mbv_index;
    /** Group index. This will remain constant until gds_mca_base_var_finalize()
        is called. This variable will be deregistered if the associated group
        is deregistered with gds_mca_base_var_group_deregister() */
    int mbv_group_index;

    /** Info level of this variable */
    gds_mca_base_var_info_lvl_t mbv_info_lvl;

    /** Enum indicating the type of the variable (integer, string, boolean) */
     gds_mca_base_var_type_t mbv_type;
    ...
    /** List of synonym names for this variable.  This *must* be a
        pointer (vs. a plain gds_list_t) because we copy this whole
        struct into a new var for permanent storage
        (gds_vale_array_append_item()), and the internal pointers in
        the gds_list_t will be invalid when that happens.  Hence, we
        simply keep a pointer to an external gds_list_t.  Synonyms
        are uncommon enough that this is not a big performance hit. */
    gds_value_array_t mbv_synonyms;

    /** Variable flags */
    gds_mca_base_var_flag_t  mbv_flags;

    /** Variable scope */
    gds_mca_base_var_scope_t  mbv_scope;

    /** Source of the current value */
    gds_mca_base_var_source_t mbv_source;
    ...
    gds_mca_base_var_enum_t *mbv_enumerator;

    /** Bind value for this variable (0 - none) */
    int mbv_bind;

    /** Storage for this variable */
    gds_mca_base_var_storage_t *mbv_storage;
 ...
 * Convenience typedef.
 */
typedef struct gds_mca_base_var_t gds_mca_base_var_t;
...
/**
 * Object declarayion for gds_mca_base_var_t
 */
GDS_CLASS_DECLARATION(gds_mca_base_var_t);
 ...
 * invoked internally (by gds_mca_base_open()) and is only documented
 ...
 * @retval GDS_ERR_OUT_OF_RESOURCE Upon failure to allocate memory.
 * @retval GDS_ERROR Upon failure to register the variable.
 *
 ...
 * gds_mca_base_var_enum_create(), and describes the valid values of an
 ...
 * gds_mca_base_var_deregister(), gds_mca_base_var_group_deregister(), or
 * gds_mca_base_var_finalize().  {enumerator} should be NULL for
 ...
 * be set using gds_mca_base_var_set_value() (i.e., the MPI_T interface).
 *
 * If MCA_BASE_VAR_FLAG_INTERNAL is set in {flags}, this variable
 * is not shown by default in the output of ompi_info.  That is,
 * this variable is considered internal to the GDS implementation
 ...
 * the description of gds_mca_base_var_scope_t for information about the
...
int gds_mca_base_var_register (const char *project_name, const char *framework_name,
                                ...
                                gds_mca_base_var_enum_t *enumerator, int bind, gds_mca_base_var_flag_t flags,
 ...
 * component.
 *
 * While quite similar to gds_mca_base_var_register(), there is one key
 ...
 * function is equivalent to gds_mca_base_var_register with component_name = "base" and
 * with the MCA_BASE_VAR_FLAG_DWG set. See gds_mca_base_var_register().
 */
 ...
 * @returns GDS_ERR_BAD_VAR If synonym_for does not reference a valid
 * variable.
 * @returns GDS_ERR_OUT_OF_RESOURCE If memory could not be allocated.
 * @returns GDS_ERROR For all other errors.
 *
 ...
 * gds_mca_base_var_*() API).  If the user sets values for both MCA
 ...
 *
 * @param vari Index returned from gds_mca_base_var_register() or
 * gds_mca_base_var_register_synonym().
 *
 ...
 * gds_mca_base_var_finalize() is called.
 *
 ...
 *
 * @return GDS_ERROR Upon failure.  The contents of value are
 * undefined.
 * @return GDS_SUCCESS Upon success. value (if not NULL) will be filled
 ...
 * the gds_mca_base_var_* interface so the source may be incorrect.
 */
 ...
 * @retval GDS_ERR_PERM If the variable is not settable.
 * @retval GDS_ERR_BAD_PARAM If the variable does not exist or has
 * been deregistered.
 * @retval GDS_ERROR On other error.
 *
 ...
 * settable.
 */
int gds_mca_base_var_set_value (int vari, const void *value, size_t size,
                                 gds_mca_base_var_source_t source,
                                 const char *source_file);
 ...
 *
 * @retval GDS_ERROR If the variable was not found.
 ...
 * gds_mca_base_var_get() and gds_mca_base_var_get_value().
 */
int gds_mca_base_var_find (const char *project_name,
 ...
 *
 * This function will print an gds_show_help() message and return
 * GDS_ERR_BAD_VAR if it finds that the two variables both have
 ...
 * not default values).
 *
 * Note that gds_show_help() allows itself to be hooked, so if this
 * happens after the aggregated gds_show_help() system is
 * initialized, the messages will be aggregated (w00t).
 *
 * @returns GDS_ERR_BAD_VAR if the two variables have sources that
 * are not MCA_BASE_VAR_SOURCE_DEFAULT.
 * @returns GDS_SUCCESS otherwise.
 */
 ...
 *
 * @returns GDS_SUCCESS If the flags are set successfully.
 * @returns GDS_ERR_BAD_PARAM If the variable is not registered.
 * @returns GDS_ERROR Otherwise
 */
int gds_mca_base_var_set_flag(int vari, gds_mca_base_var_flag_t flag,
                               bool set);
 ...
 *
 * @retval GDS_SUCCESS Upon success.
 * @retval gds error code Upon failure.
 *
 ...
 * gds_mca_base_var_deregister() has no impact on the variable count. The count
 * returned is equal to the number of calls to gds_mca_base_var_register with
 ...
 *
 * This function is similar to gds_mca_base_var_dump() except that
 * its output is in terms of an argv-style array of key=value
 * strings, suitable for using in an environment.
 */
int gds_mca_base_var_build_env(char ***env, int *num_env,
                                bool internal);
 ...
 *
 * @returns GDS_SUCCESS This function never fails.
 *
 * This function shuts down the MCA variable repository and frees all
 * associated memory.  No other gds_mca_base_var*() functions can be
...
typedef enum {
    ...
    GDS_MCA_BASE_VAR_DUMP_READABLE = 0,
    /* Dump easily parsable strings */
    GDS_MCA_BASE_VAR_DUMP_PARSABLE = 1,
    /* Dump simple name=value string */
    GDS_MCA_BASE_VAR_DUMP_SIMPLE   = 2
} gds_mca_base_var_dump_type_t;
