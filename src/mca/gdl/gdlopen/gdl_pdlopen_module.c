 ...
 */
static void do_gdlopen(const char *fname, int flags,
...
static int gdlopen_open(const char *fname, bool use_ext, bool private_namespace,
                       gds_gdl_handle_t **handle, char **err_msg)
{
    ...
    *handle = NULL;
    ...
    if (use_ext && NULL != fname) {
        ...
        for (i = 0, ext = mca_gdl_gdlopen_component.filename_suffixes[i];
             NULL != ext;
             ext = mca_gdl_gdlopen_component.filename_suffixes[++i]) {
            char *name;
            ...
            /* Yes, the file exists -- try to dlopen it.  If we can't
               dlopen it, bail. */
            do_gdlopen(name, flags, &local_handle, err_msg);
            free(name);
            break;
        }
    }
    ...
    /* Otherwise, the caller does not want to use filename extensions,
       so just use the single filename that the caller provided */
        ...
        do_gdlopen(fname, flags, &local_handle, err_msg);
    }

    if (NULL != local_handle) {
        *handle = calloc(1, sizeof(gds_gdl_handle_t));
    ...
    return (NULL != local_handle) ? GDS_SUCCESS : GDS_ERROR;
}


static int gdlopen_lookup(gds_gdl_handle_t *handle, const char *symbol,
...
static int gdlopen_close(gds_gdl_handle_t *handle)
{
    assert(handle);
 ...
 * on each one.
 */
static int gdlopen_foreachfile(const char *search_path,
                              ...
                              void *data)
{
    ...
    dirs = gds_argv_split(search_path, GDS_ENV_SEP);
    for (int i = 0; NULL != dirs && NULL != dirs[i]; ++i) {

        dp = opendir(dirs[i]);
        if (NULL == dp) {
            ret = GDS_ERR_IN_ERRNO;
            goto error;
        }
        ...
        while (NULL != (de = readdir(dp))) {
            ...
            if (!found) {
                gds_argv_append_nosize(&good_files, abs_name);
            }
            free(abs_name);
        }
        closedir(dp);
 ...
 * Module definition
 */
gds_gdl_base_module_t gds_gdl_gdlopen_module = {
    .open = gdlopen_open,
    .lookup = gdlopen_lookup,
    .close = gdlopen_close,
    .foreachfile = gdlopen_foreachfile
};
