 ...
 *   given time (i.e., when an gds timer event expires).
 ...
 *
 * There is an gds_hotel_init() function to create a hotel, but no
...
typedef void (*gds_hotel_eviction_callback_fn_t)(struct gds_hotel_t *hotel,
                                                  int room_num,
                                                  void *occupant);

/* Note that this is an internal data structure; it is not part of the
   public gds_hotel interface.  Public consumers of gds_hotel
...
typedef struct {
    void *occupant;
    gds_event_t eviction_timer_event;
} gds_hotel_room_t;
...
GDS_CLASS_DECLARATION(gds_hotel_t);
 ...
 *
 * @return GDS_SUCCESS if all initializations were succesful. Otherwise,
 *  the error indicate what went wrong in the function.
 */
int gds_hotel_init(gds_hotel_t *hotel, int num_rooms,
                                  gds_event_base_t *evbase,
 ...
 *
 * @return GDS_SUCCESS if the occupant is successfully checked in,
 * and the room parameter will contain a valid value.
 * @return GDS_ERR_TEMP_OUT_OF_RESOURCE is the hotel is full.  Try
 * again later.
 */
static inline int gds_hotel_checkin(gds_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    gds_hotel_room_t *room;

    /* Do we have any rooms available? */
    if (GDS_UNLIKELY(hotel->last_unoccupied_room < 0)) {
...
/**
 * Same as gds_hotel_checkin(), but slightly optimized for when the
 * caller *knows* that there is a room available.
 */
static inline void gds_hotel_checkin_with_res(gds_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    gds_hotel_room_t *room;
 ...
 */
static inline void gds_hotel_checkout(gds_hotel_t *hotel, int room_num)
{
    gds_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (GDS_LIKELY(NULL != room->occupant)) {
        /* Do not change this logic without also changing the same
           logic in gds_hotel_checkout_and_return_occupant() and
           gds_hotel.c:local_eviction_callback(). */
        room->occupant = NULL;
        if (NULL != hotel->evbase) {
 ...
 */
static inline void gds_hotel_checkout_and_return_occupant(gds_hotel_t *hotel, int room_num, void **occupant)
{
    gds_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (GDS_LIKELY(NULL != room->occupant)) {
        gds_output (10, "checking out occupant %p from room num %d", room->occupant, room_num);
        /* Do not change this logic without also changing the same
           logic in gds_hotel_checkout() and
           gds_hotel.c:local_eviction_callback(). */
 ...
 *
 */
static inline bool gds_hotel_is_empty (gds_hotel_t *hotel)
{
 ...
 */
static inline void gds_hotel_knock(gds_hotel_t *hotel, int room_num, void **occupant)
{
    gds_hotel_room_t *room;
    ...
    *occupant = NULL;
    ...
    room = &(hotel->rooms[room_num]);
    if (GDS_LIKELY(NULL != room->occupant)) {
        gds_output (10, "occupant %p in room num %d responded to knock", room->occupant, room_num);
        *occupant = room->occupant;
    }
}
