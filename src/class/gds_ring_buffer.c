...
static void gds_ring_buffer_construct(gds_ring_buffer_t *);
static void gds_ring_buffer_destruct(gds_ring_buffer_t *);

GDS_CLASS_INSTANCE(gds_ring_buffer_t, gds_object_t,
                    gds_ring_buffer_construct,
                    gds_ring_buffer_destruct);

/*
 * gds_ring_buffer constructor
 */
static void gds_ring_buffer_construct(gds_ring_buffer_t *ring)
{
    ring->head = 0;
    ring->tail = -1;
    ring->size = 0;
...
/*
 * gds_ring_buffer destructor
 */
static void gds_ring_buffer_destruct(gds_ring_buffer_t *ring)
{
    if( NULL != ring->addr) {
        free(ring->addr);
        ring->addr = NULL;
 ...
 */
int gds_ring_buffer_init(gds_ring_buffer_t* ring, int size)
{
    /* check for errors */
    if (NULL == ring) {
...
void* gds_ring_buffer_push(gds_ring_buffer_t *ring, void *ptr)
{
    char *p=NULL;
...
void* gds_ring_buffer_pop(gds_ring_buffer_t *ring)
{
    char *p=NULL;
 ...
 void* gds_ring_buffer_poke(gds_ring_buffer_t *ring, int i)
 {
    char *p=NULL;
    int offset;
