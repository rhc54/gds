...
static void gds_value_array_construct(gds_value_array_t* array)
{
...
static void gds_value_array_destruct(gds_value_array_t* array)
{
...
GDS_CLASS_INSTANCE(
    gds_value_array_t,
    gds_object_t,
    gds_value_array_construct,
    gds_value_array_destruct
);
...
int gds_value_array_set_size(gds_value_array_t* array, size_t size)
{
    ...
    if(array->array_item_sizeof == 0) {
        gds_output(0, "gds_value_array_set_size: item size must be initialized");
