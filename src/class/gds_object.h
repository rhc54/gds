 ...
 * the class like this:
 * @code
 *   GDS_CLASS_INSTANCE(sally_t, parent_t, sally_construct, sally_destruct);
 * @endcode
 * This macro actually expands to
 * @code
 *   gds_class_t sally_t_class = {
 *     "sally_t",
 *     GDS_CLASS(parent_t),  // pointer to parent_t_class
 *     sally_construct,
 *     sally_destruct,
 *     0, 0, NULL, NULL,
 *     sizeof ("sally_t")
 ...
 * the GDS_CLASS_DECLARATION macro as shown above.
 *
 ...
 *
 * To create a instance of a class (an object) use GDS_NEW:
 * @code
 *   sally_t *sally = GDS_NEW(sally_t);
 * @endcode
 * which allocates memory of sizeof(sally_t) and runs the class's
 * constructors.
 *
 * Use GDS_RETAIN, GDS_RELEASE to do reference-count-based
 * memory management:
 * @code
 *   GDS_RETAIN(sally);
 *   GDS_RELEASE(sally);
 ...
 *   sally_t sally;
 *
 *   GDS_CONSTRUCT(&sally, sally_t);
 * @endcode
 * The retain/release model is not necessary here, but before the
 * object goes out of scope, GDS_DESTRUCT should be run to release
...
#define GDS_OBJ_MAGIC_ID ((0xdeafbeedULL << 32) + 0xdeafbeedULL)
#endif

/* typedefs ***********************************************************/

typedef struct gds_object_t gds_object_t;
typedef struct gds_class_t gds_class_t;
typedef void (*gds_construct_t) (gds_object_t *);
typedef void (*gds_destruct_t) (gds_object_t *);
...
struct gds_class_t {
    ...
    gds_class_t *cls_parent;       /**< parent class descriptor */
    gds_construct_t cls_construct; /**< class constructor */
    gds_destruct_t cls_destruct;   /**< class destructor */
    int cls_initialized;            /**< is class initialized */
    int cls_depth;                  /**< depth of class hierarchy tree */
    gds_construct_t *cls_construct_array;
                                    /**< array of parent class constructors */
    gds_destruct_t *cls_destruct_array;
 ...
 */
#if GDS_ENABLE_DEBUG
#define GDS_OBJ_STATIC_INIT(BASE_CLASS) { GDS_OBJ_MAGIC_ID, GDS_CLASS(BASE_CLASS), 1, __FILE__, __LINE__ }
#else
#define GDS_OBJ_STATIC_INIT(BASE_CLASS) { GDS_CLASS(BASE_CLASS), 1 }
#endif
...
#endif
    gds_class_t *obj_class;            /**< class descriptor */
 ...
 */
#define GDS_CLASS(NAME)     (&(NAME ## _class))
 ...
 * Put this in NAME.c
 */
#define GDS_CLASS_INSTANCE(NAME, PARENT, CONSTRUCTOR, DESTRUCTOR)       \
    gds_class_t NAME ## _class = {                                     \
        # NAME,                                                         \
        GDS_CLASS(PARENT),                                              \
        (gds_construct_t) CONSTRUCTOR,                                 \
        (gds_destruct_t) DESTRUCTOR,                                   \
        0, 0, NULL, NULL,                                               \
        sizeof(NAME)                                                    \
    }
 ...
 * Put this in NAME.h
 */
#define GDS_CLASS_DECLARATION(NAME)             \
    extern gds_class_t NAME ## _class
 ...
 */
static inline gds_object_t *gds_obj_new(gds_class_t * cls);
#if GDS_ENABLE_DEBUG
static inline gds_object_t *gds_obj_new_debug(gds_class_t* type, const char* file, int line)
{
    gds_object_t* object = gds_obj_new(type);
    object->obj_magic_id = GDS_OBJ_MAGIC_ID;
    object->cls_init_file_name = file;
    object->cls_init_lineno = line;
    return object;
}
#define GDS_NEW(type)                                   \
    ((type *)gds_obj_new_debug(GDS_CLASS(type), __FILE__, __LINE__))
#else
#define GDS_NEW(type)                                   \
    ((type *) gds_obj_new(GDS_CLASS(type)))
#endif  /* GDS_ENABLE_DEBUG */
 ...
 */
#if GDS_ENABLE_DEBUG
#define GDS_RETAIN(object)                                              \
    do {                                                                \
        assert(NULL != ((gds_object_t *) (object))->obj_class);        \
        assert(GDS_OBJ_MAGIC_ID == ((gds_object_t *) (object))->obj_magic_id); \
        gds_obj_update((gds_object_t *) (object), 1);                 \
        assert(((gds_object_t *) (object))->obj_reference_count >= 0); \
    } while (0)
#else
#define GDS_RETAIN(object)  gds_obj_update((gds_object_t *) (object), 1);
#endif
 ...
 */
#if GDS_ENABLE_DEBUG
#define GDS_REMEMBER_FILE_AND_LINENO( OBJECT, FILE, LINENO )    \
    do {                                                        \
        ((gds_object_t*)(OBJECT))->cls_init_file_name = FILE;  \
        ((gds_object_t*)(OBJECT))->cls_init_lineno = LINENO;   \
    } while (0)
#define GDS_SET_MAGIC_ID( OBJECT, VALUE )                       \
    do {                                                        \
        ((gds_object_t*)(OBJECT))->obj_magic_id = (VALUE);     \
    } while (0)
#else
#define GDS_REMEMBER_FILE_AND_LINENO( OBJECT, FILE, LINENO )
#define GDS_SET_MAGIC_ID( OBJECT, VALUE )
 ...
 */
#if GDS_ENABLE_DEBUG
#define GDS_RELEASE(object)                                             \
    do {                                                                \
        assert(NULL != ((gds_object_t *) (object))->obj_class);        \
        assert(GDS_OBJ_MAGIC_ID == ((gds_object_t *) (object))->obj_magic_id); \
        if (0 == gds_obj_update((gds_object_t *) (object), -1)) {     \
            GDS_SET_MAGIC_ID((object), 0);                              \
            gds_obj_run_destructors((gds_object_t *) (object));       \
            GDS_REMEMBER_FILE_AND_LINENO( object, __FILE__, __LINE__ ); \
            free(object);                                               \
            object = NULL;                                              \
        }                                                               \
    } while (0)
...
#define GDS_CONSTRUCT(object, type)                             \
do {                                                            \
    GDS_CONSTRUCT_INTERNAL((object), GDS_CLASS(type));          \
} while (0)

#define GDS_CONSTRUCT_INTERNAL(object, type)                        \
do {                                                                \
    GDS_SET_MAGIC_ID((object), GDS_OBJ_MAGIC_ID);              \
    if (0 == (type)->cls_initialized) {                             \
        gds_class_initialize((type));                              \
    }                                                               \
    ((gds_object_t *) (object))->obj_class = (type);               \
    ((gds_object_t *) (object))->obj_reference_count = 1;          \
    gds_obj_run_constructors((gds_object_t *) (object));          \
    GDS_REMEMBER_FILE_AND_LINENO( object, __FILE__, __LINE__ ); \
} while (0)
...
GDS_CLASS_DECLARATION(gds_object_t);
 ...
 */
void gds_class_initialize(gds_class_t *);
 ...
 */
int gds_class_finalize(void);

/**
 * Run the hierarchy of class constructors for this object, in a
 * parent-first order.
 *
 * Do not use this function directly: use GDS_CONSTRUCT() instead.
 *
 ...
 */
static inline void gds_obj_run_constructors(gds_object_t * object)
{
    gds_construct_t* cls_construct;
 ...
 * parent-last order.
 *
 * Do not use this function directly: use GDS_DESTRUCT() instead.
 *
 * @param size          Pointer to the object.
 */
static inline void gds_obj_run_destructors(gds_object_t * object)
{
    gds_destruct_t* cls_destruct;
 ...
 * constructor.
 *
 * Do not use this function directly: use GDS_NEW() instead.
 *
...
{
    gds_object_t *object;
    assert(cls->cls_sizeof >= sizeof(gds_object_t));

    object = (gds_object_t *) malloc(cls->cls_sizeof);
    if (0 == cls->cls_initialized) {
        gds_class_initialize(cls);
    }
    if (NULL != object) {
        object->obj_class = cls;
        object->obj_reference_count = 1;
        gds_obj_run_constructors(object);
    }
    return object;
}
 ...
 * macros GDS_RETAIN and GDS_RELEASE
 *
 * @param object        Pointer to the object
 * @param inc           Increment by which to update reference count
 * @return              New value of the reference count
 */
static inline int gds_obj_update(gds_object_t *object, int inc) __gds_attribute_always_inline__;
static inline int gds_obj_update(gds_object_t *object, int inc)
{
