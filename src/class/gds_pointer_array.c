...
static void gds_pointer_array_construct(gds_pointer_array_t *);
static void gds_pointer_array_destruct(gds_pointer_array_t *);
static bool grow_table(gds_pointer_array_t *table, int soft, int hard);

GDS_CLASS_INSTANCE(gds_pointer_array_t, gds_object_t,
                   gds_pointer_array_construct,
                   gds_pointer_array_destruct);

/*
 * gds_pointer_array constructor
 */
static void gds_pointer_array_construct(gds_pointer_array_t *array)
{
    array->lowest_free = 0;
    array->number_free = 0;
    array->size = 0;
...
/*
 * gds_pointer_array destructor
 */
static void gds_pointer_array_destruct(gds_pointer_array_t *array)
{
    /* free table */
    if( NULL != array->addr) {
        free(array->addr);
 ...
 */
gds_status_t gds_pointer_array_init(gds_pointer_array_t* array,
 ...
 *
 * @param table Pointer to gds_pointer_array_t object (IN)
 * @param ptr Pointer to be added to table    (IN)
 *
 * @return Array index where ptr is inserted or GDS_ERROR if it fails
 */
int gds_pointer_array_add(gds_pointer_array_t *table, void *ptr)
{
    int i, index;
 ...
 */
gds_status_t gds_pointer_array_set_item(gds_pointer_array_t *table, int index,
                                          void * value)
{
    assert(table != NULL);
...
#if 0
    gds_output(0,"gds_pointer_array_set_item: OUT: "
 ...
 */
bool gds_pointer_array_test_and_set_item (gds_pointer_array_t *table,
...
#if 0
    gds_output(0,"gds_pointer_array_test_and_set_item: IN:  "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
#if 0
    gds_output(0,"gds_pointer_array_test_and_set_item: OUT: "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
gds_status_t gds_pointer_array_set_size(gds_pointer_array_t *array, int new_size)
{
