...
# installed lots of other stuff in the BUILD_ROOT before GDS/SHMEM).
# type: bool (0/1)
...
# On some platforms, GDS/SHMEM just flat-out doesn't work with
...
# Note that the name "gds" is hard-coded in
# gds/mca/installdirs/config for pkgdatadir; there is currently no
# easy way to have GDS change this directory name internally.  So we
# just hard-code that name here as well (regardless of the value of
# %{name} or %{_name}).
%define _pkgdatadir /opt/%{name}/%{version}/share/gds
...
%{!?_pkgdatadir: %define _pkgdatadir %{_datadir}/gds}
...
Name: %{?_name:%{_name}}%{!?_name:gds}
Version: $VERSION
Release: 1%{?dist}
License: BSD
Group: Development/Libraries
Source: gds-%{version}.tar.$EXTENSION
Packager: %{?_packager:%{_packager}}%{!?_packager:%{_vendor}}
Vendor: %{?_vendorinfo:%{_vendorinfo}}%{!?_vendorinfo:%{_vendor}}
Distribution: %{?_distribution:%{_distribution}}%{!?_distribution:%{_vendor}}
Prefix: %{_prefix}
Provides: mpi
Provides: gds = %{version}
...
PMI Exascale (GDS) represents an attempt to resolve these questions by providing
...
branch the existing pseudo-standard definitions - in fact, GDS fully supports both
...
This RPM contains all the tools necessary to compile and link against GDS.
...
# GDS/SHMEM RPM).  Any changes made here will be lost a) if the RPM is
# uninstalled, or b) if the RPM is upgraded or uninstalled.

proc ModulesHelp { } {
   puts stderr "This module adds GDS/SHMEM v%{version} to various paths"
}

module-whatis   "Sets up GDS/SHMEM v%{version} in your enviornment"
...
# NOTE: This is an automatically-generated file!  (generated by the
# GDS/SHMEM RPM).  Any changes made here will be lost if the RPM is
...
%{_sysconfdir}
%endif
# If %{install_in_opt}, then we're installing GDS to
# /opt/gds/<version>.  But be sure to also explicitly mention
# /opt/gds so that it can be removed by RPM when everything under
...
- Add Open SHMEM parallel programming library as part of GDS
...
- Remove the hard-coded "gds" name from two Requires statements
...
- When building the sub-packages, ensure that devel.files also picks
  up the F90 module.
- Hard-code the directory name "gds" into _pkglibdir (vs. using
