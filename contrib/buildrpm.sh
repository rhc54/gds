...
unset first
echo "--> Found GDS version: $version"
...
specdest="$rpmtopdir/SPECS/gds-$version.spec"
...
if test "$build_single" = "yes"; then
    echo "--> Building the single GDS RPM"
...
if test "$build_multiple" = "yes"; then
    echo "--> Building the multiple GDS RPM"
