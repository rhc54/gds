...
gds_dir=${WORKSPACE}/${prefix}/install
...
if [ "$jenkins_test_build" = "yes" ]; then
    ...
    # build gds
    $autogen_script
    echo ./configure --prefix=$gds_dir $configure_args | bash -xeE
...
if [ "$jenkins_test_src_rpm" = "yes" ]; then
    ...
    $autogen_script
    echo ./configure --prefix=$gds_dir $configure_args | bash -xeE || exit 11

    if [ -x /usr/bin/dpkg-buildpackage ]; then
        echo "Do not support GDS on debian"
    else
        echo "Building GDS src.rpm"
        rm -rf $tarball_dir
        mkdir -p $tarball_dir
        ...
        tarball_src=$(ls -1 $tarball_dir/gds-*.tar.bz2|sort -r|head -1)

        echo "Building GDS bin.rpm"
...
if [ -n "$JENKINS_RUN_TESTS" -a "$JENKINS_RUN_TESTS" -ne "0" ]; then
    ...
    # build gds
    $autogen_script
    echo ./configure --prefix=$gds_dir $configure_args --disable-visibility | bash -xeE
    make $make_opt install
    ...
    test_exec='./gds_test -n 4 --ns-dist 3:1 --fence "[db | 0:0-2;1:3]"'
    check_result "blocking fence w/ data all" "$test_exec"
    test_exec='./gds_test -n 4 --ns-dist 3:1 --fence "[db | 0:;1:3]"'
    check_result "blocking fence w/ data all" "$test_exec"
    test_exec='./gds_test -n 4 --ns-dist 3:1 --fence "[db | 0:;1:]"'
    check_result "blocking fence w/ data all" "$test_exec"

    # 1 non-blocking fence without data exchange among processes from the 1st namespace
    test_exec='./gds_test -n 4 --ns-dist 3:1 --fence "[0:]"'
    ...
    test_exec='./gds_test -n 4 --ns-dist 3:1 --fence "[b | 0:]"'
    ...
    test_exec='./gds_test -n 4 --ns-dist 3:1 --fence "[d | 0:]" --noise "[0:0,1]"'
    ...
    test_exec='./gds_test -n 4 --job-fence -c'
...
#    check_result "mix fence" $test_exec
    ...
    if [ "$jenkins_test_vg" = "yes" ]; then
        ...
        valgrind $vg_opt  ./gds_test -n 4 --timeout 60 --ns-dist 3:1 --fence "[db | 0:;1:3]"

        valgrind $vg_opt  ./gds_test -n 4 --timeout 60 --job-fence -c

        valgrind $vg_opt  ./gds_test -n 2 --timeout 60 --test-publish

        valgrind $vg_opt  ./gds_test -n 2 --timeout 60 --test-spawn

        valgrind $vg_opt  ./gds_test -n 2 --timeout 60 --test-connect

        valgrind $vg_opt  ./gds_test -n 5 --timeout 60 --test-resolve-peers --ns-dist "1:2:2"
