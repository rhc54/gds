...
# svn repository uri
master_code_uri=https://github.com/open-mpi/gds.git
master_raw_uri=https://raw.github.com/open-mpi/gds
release_code_uri=https://github.com/open-mpi/gds-release.git
release_raw_uri=https://raw.github.com/open-mpi/gds-release

# where to put built tarballs
outputroot=/l/osl/www/www.open-mpi.org/software/gds/nightly
...
build_root=/home/mpiteam/gds/nightly-tarball-build-root

# Coverity stuff
coverity_token=`cat $HOME/coverity/gds-token.txt`
...
for branch in $branches; do
    ...
    if test ! $? -eq 0 ; then
        echo "wget of GDS nightly tarball create script failed."
    ...
    module load "autotools/gds-$branch"
    module load "libevent/gds-$branch"
    ...
    if test "$prev_snapshot" != "$latest_snapshot" && \
        ...
        echo "$outputroot/$branch/gds-$latest_snapshot.tar.bz2" >> $pending_coverity
    else
...
for tarball in `cat $pending_coverity`; do
    ...
    $HOME/scripts/gds-nightly-coverity.pl \
        --filename=$tarball \
