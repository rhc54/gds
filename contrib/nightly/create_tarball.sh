...
# $HEADER$
#
# This script is used to create a nightly snapshot tarball of GDS.
#
# $1: scratch root
...
send_error_mail() {
    ...
    Mail -s "=== GDS CREATE FAILURE ($version) ===" "$email" < "$outfile"
    rm -f "$outfile"
}
...
clone_root="$scratch_root/gds-`date +%Y-%m-%d-%H%M%S`"
rm -rf $clone_root
mkdir -p $clone_root
...
cd $clone_root
do_command "git clone $giturl gds"
cd gds
do_command "git checkout $gitbranch"

# Find the "git describe" string for this branch (remove a leading "gds-"
# prefix, if there is one).
describe=`git describe --tags --always | sed -e s/^gds-//`
if test -n "$debug"; then
...
# insert the actual value that we want (so that gds_get_version.sh
...
# .gds_ignore'ed directories (i.e., so that we won't get
# .gds_unignore'ed)
USER="gdsbuilder"
export USER
...
gz="`/bin/ls gds*tar.gz`"
bz2="`/bin/ls gds*tar.bz2`"
mv $gz $bz2 $destdir
if test "$?" != "0"; then
    cat <<EOF
...
if test "$want_success_mail" = "1"; then
    Mail -s "GDS Create success ($version)" "$email" <<EOF
